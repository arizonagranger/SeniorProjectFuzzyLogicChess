...
def move_lists(piece):
    ...
    moves = request.post(url = URL + "actions-for?coord=" + +boardCord[piece[0]]+str(piece[1]+1))
...
def random_move(team):
    ...
    while len(delegates) > 0:
        ...
        for piece in del_pieces:
            del_moves.append(request.post(url = URL + "actions-for?coord=" + piece).text.split("\n"))
        random_move = del_moves[random.randrange(len(del_moves))]
        submitMove(request.post(url = URL + "actions-for?coord=" + random_move)) ##this would be where the ai would send to the move to rules and does everything with the board state
