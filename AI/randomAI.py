...
def random_move(team):
    ...
    while len(delegates) > 0:
        ...
        for piece in del_pieces:
            del_moves.append(requests.post(url = URL + "actions-for?coord=" + piece).text.split("\n"))
        random_move = del_moves[random.randrange(len(del_moves))]
        requests.post(url = URL + "actions-for?coord=" + random_move) ##this would be where the ai would send to the move to rules and does everything with the board state
